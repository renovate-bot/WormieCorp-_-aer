    ...
    fn run<'a, T: RunnerCombiner + Debug>(
        ...
        let runner_template = format!(
            "$ErrorActiorPreference = 'Stop'; $InformationPreference = 'Continue'; \
             ...
             [int]$exitCode = 1; }} }}; Write-Host \"## PKG-SCRIPT-RUNNER:START ##\"; Write-Host \
             ($data | ConvertTo-Json); Write-Host \"## PKG-SCRIPT-RUNNER:END ##\"; if ($exitCode \
        ...
        let mut run_data = String::new();
        {
            ...
            debug!("PKG-SCRIPT-RUNNER STDOUT:");

            for line in stdout.lines() {
                match line.trim() {
                    "## PKG-SCRIPT-RUNNER:START ##" => in_data = true,
                    "## PKG-SCRIPT-RUNNER:END ##" => in_data = false,
                    line => {
                        if in_data {
...

        {
            ...
            debug!("PKG-SCRIPT-RUNNER STDERR:");
            let mut fail = false;
...
mod tests {
    ...
    fn run_should_allow_changes_to_project_url() {
        ...
        assert_eq!(
            ...
            &Url::parse("https://github.com/WormieCorp/pkg-upd").unwrap()
        );
    }
